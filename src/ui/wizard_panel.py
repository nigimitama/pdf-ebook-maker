"""WizardPanel — 5-step accordion wizard for the PDF Ebook Maker workflow.
...
pdf_requested():           user clicked generate in Step 5.
...
from .constants import BG_GRAY, BORDER, BORDER_LIGHT, CORAL, TEXT_MUTED, TEXT_PRI, TEXT_SEC
...
class WizardPanel(QWidget):
    """Scrollable wizard panel with 5 accordion steps."""
    ...
    def _make_step1_content(self) -> QWidget:
        ...
        # Shown while expanding paths in background
        ...
        # Shown after loading completes (clear button only — count is in step card header)
        clear_row = QWidget()
    ...
    def _make_step3_content(self) -> QWidget:
        ...
        next3_btn = QPushButton("次へ  ›")
        next3_btn.setFixedHeight(44)
        next3_btn.setFixedWidth(140)
        next3_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        next3_btn.setStyleSheet(f"""
            QPushButton {{
                background:{CORAL};color:white;
                border:none;border-radius:10px;
            ...
            QPushButton:hover {{ background:#ff5252; }}
        """)
        next3_btn.clicked.connect(self._on_step3_next)
        btn_layout.addWidget(next3_btn)
    ...
    def _on_step3_next(self) -> None:
        self._step3.set_completed("目次確認済み")
